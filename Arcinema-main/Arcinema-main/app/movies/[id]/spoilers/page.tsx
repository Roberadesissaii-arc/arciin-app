...
function formatSpoilerContent(content: string): string {
  ...
  const formatted = escaped
    // Quoted text (movie/show titles) - Give them indigo color
...
export default function MovieSpoilersPage() {
  ...
  // Check if spoilers should be available (movie must be released for at least 3 months)
  const isSpoilerAvailable = (releaseDate: string): boolean => {
    const release = new Date(releaseDate);
    const now = new Date();
    const threeMonthsAgo = new Date();
    threeMonthsAgo.setMonth(threeMonthsAgo.getMonth() - 3);

    // Movie must be released (not in future) and at least 3 months old
  ...
  const handleLoadSpoilers = async () => {
    ...
    try {
      const result = await spoilerService.getMovieSpoilers(
        movie.title,
      ...
      if (result.success) {
        ...
        const key = `spoilers-movie-${normalizedTitle}-${year}`;
        setCacheKey(key);
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <h1 className="text-xl text-white mb-2">Error Loading Movie</h1>
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="relative z-10 max-w-4xl mx-auto p-4 md:p-6 lg:p-8">
        {isSpoilerAvailable(movie.release_date) ? (
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-6 mb-8">
            <div className="mb-4">
              ...
              <p className="text-gray-400">
                This section contains major plot details and spoilers for {movie.title}. 
                Only continue if you want to know everything about the movie's story.
              </p>
            </div>
          ...
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-6 mb-8">
            ...
            <p className="text-gray-400">
              {new Date(movie.release_date) > new Date() 
                ? `This movie hasn't been released yet. It will be available on ${new Date(movie.release_date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}. Spoilers will become available 3 months after release.`
                : `This movie was recently released on ${new Date(movie.release_date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}. Spoilers will become available 3 months after release to avoid spoiling the experience for viewers.`
              }
            </p>
          </div>
        )}
