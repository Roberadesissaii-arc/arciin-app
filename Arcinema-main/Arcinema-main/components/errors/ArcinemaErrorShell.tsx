...
/** Same particle field as `app/not-found.tsx` (16 particles). */
const PARTICLES = [
...
interface ArcinemaErrorShellProps {
  code: string;
  title: string;
  description: string;
  children?: ReactNode;
  /** Omit bottom “Arcinema” wordmark (e.g. inside app chrome). */
  hideBrand?: boolean;
}

/**
 * Indigo full-screen shell matching `app/not-found.tsx` (for in-app detail errors).
 */
export function ArcinemaErrorShell({
  code,
  title,
  description,
  children,
  hideBrand = false,
}: ArcinemaErrorShellProps) {
  return (
    <div
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 max-w-lg">
        ...
        <h1 className="text-[22px] font-bold text-white mb-2 tracking-tight">{title}</h1>
        <p
